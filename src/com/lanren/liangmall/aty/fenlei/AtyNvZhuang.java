...
public class AtyNvZhuang extends Activity {

	private List<GoodsEntity> nvList = new ArrayList<GoodsEntity>();
	private int classify;
	private NanZAdapter NanAdapter;
	private ListView nvListView;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.aty_fenlei_nvzhuang);
		nvListView = (ListView) findViewById(R.id.fenlei_nvzhuang);
	...
	// 解析商品的JSON数据
			...
			NetHttpData.getHttpDao().getFenLeiGoods(classify + "", new JsonHttpResponseHandler("utf-8") {
				...
				public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
					...
					try {
						...
						NanAdapter = new NanZAdapter(AtyNvZhuang.this, nvList);
						nvListView.setAdapter(NanAdapter);
		...
		public void dianJi(){
			nvListView.setOnItemClickListener(new OnItemClickListener() {

				@Override
				public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					GoodsEntity g = nvList.get(position);
					Intent in = new Intent(AtyNvZhuang.this,AtyXiangQing.class);
					Bundle b = new Bundle();
